...
public class Chess {
	...
	boolean whiteCheck;
	boolean blackCheck;
	private static boolean alreadytold=false;
		...
		if(rawInputs.length > 1) {
			if(Parser.validInput(rawInputs[0]) && Parser.validInput(rawInputs[1])  ) {
				...
				if(rawInputs.length >= 3 ) {
					if(Parser.validType(rawInputs[2]) && !(rawInputs[2].contains("N") || rawInputs[2].contains("B") || rawInputs[2].contains("R") || rawInputs[2].contains("Q"))  )
						return ERROR;
	...
	/**
	 * Displays correct prompt in the case of a player resigning.
	 * @param player
	 * @author diegocastellanos
	 */
